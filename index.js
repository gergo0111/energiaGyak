...
app.post('/energy-drinks', async (req, res) => {
    ...
    if (!name)
        errors.push({ name: "A 'name' mező megadása kötelező." });
    if (!brand)
        errors.push({ brand: "A 'brand' mező megadása kötelező." });
    if (caffeine === undefined)
        errors.push({ caffeine: "A 'caffeine' mező megadása kötelező." });
    // Típusellenőrzés
    ...
    try {
        const [result] = await db.execute(`INSERT INTO energy_drinks (name, brand, caffeine, sugar_free, created_at, updated_at)
			 VALUES (?, ?, ?, ?, NOW(), NOW())`, [name, brand, caffeine, sugar_free ?? false]);
        // @ts-ignore
...
app.put('/energy-drinks/:id', async (req, res) => {
    ...
    try {
        const [result] = await db.execute(`UPDATE energy_drinks SET name=?, brand=?, caffeine=?, sugar_free=?, updated_at=NOW() WHERE id=?`, [name, brand, caffeine, sugar_free, id]);
        // @ts-ignore
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: "Energiaital nem található." });
        }
